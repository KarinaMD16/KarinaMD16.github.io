...
export function createCVView(person, emailJsConfig = null) {
...
if (projectsWrap) {
  ...
  const getProfileEmail = (items) => {
    for (const item of items) {
      const href = String(item?.href || "").trim();
      if (href.toLowerCase().startsWith("mailto:")) {
        const emailFromHref = href.slice(7).split("?")[0].trim();
  ...
  if (contactSection) {
    ...
    if (form) {
      ...
      const profileEmail = getProfileEmail(contactItems);
      ...
      form.addEventListener("submit", async (ev) => {
        ...
        const msg = String(form.elements.message?.value || "").trim();

        if (!profileEmail) {
          showMessage("This profile does not have a contact email configured.");
          return;
        }
        ...
        if (!emailjs || !serviceId || !templateId) {
          showMessage("Email service is not configured yet.");
          return;
        }
